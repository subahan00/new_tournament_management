...
import { Swords, GitMerge, BarChart3, Award, Zap, Users, ShieldQuestion, ChevronLeft } from 'lucide-react';
...
const ViewPage = () => {
    ...
    const options = [
        ...
        { label: "Player Directory", icon: Users, path: "#" },
        { label: "FAQs & Support", icon: ShieldQuestion, path: "" },
    ];

    return (
        <div className="min-h-screen flex flex-col modern-bg text-white overflow-x-hidden">
            {/* Font links updated to Orbitron, Inter, and Poppins */}
            ...
            <header className="fixed top-0 left-0 w-full z-50 p-4">
                <Link to="/" className="inline-flex items-center space-x-2 text-purple-300 hover:text-gold-300 transition-colors duration-300 group glass-header-light p-2 rounded-lg">
                    ...
                    <span className="font-poppins font-semibold text-sm tracking-wide">Back to Home</span>
                </Link>
            </header>
                    ...
                    {options.map((opt, index) => (
                        <InteractiveCard key={opt.label} className="group" animationDelay={`${index * 100}ms`}>
                            <Link to={opt.path} className="modern-info-card h-full">
                                ...
                                <h2 className="modern-card-title">{opt.label}</h2>
                            </Link>
                        </InteractiveCard>
                    ))}
                </div>
            ...
            <style jsx global>{`
                ...
                .modern-cta-button { position: relative; padding: 1.25rem 3rem; background: linear-gradient(135deg, var(--gold-main) 0%, var(--gold-dark) 100%); color: var(--purple-dark); border: none; border-radius: 16px; font-family: 'Inter', sans-serif; font-weight: 700; font-size: 1.125rem; cursor: pointer; transition: all 0.4s cubic-bezier(0.23, 1, 0.32, 1); overflow: hidden; box-shadow: 0 12px 40px rgba(255, 223, 128, 0.3), inset 0 2px 4px rgba(255, 255, 255, 0.4); text-decoration: none; letter-spacing: 0.025em; }
                ...
                .modern-info-card::before { content: ''; position: absolute; inset: 0; background: linear-gradient(135deg, rgba(255, 223, 128, 0.05) 0%, transparent 50%, rgba(255, 223, 128, 0.05) 100%); opacity: 0; transition: opacity 0.4s ease; pointer-events: none; }
                ...
                .modern-card-title { font-family: 'Orbitron', sans-serif; font-size: 1.75rem; font-weight: 700; color: var(--gold-main); line-height: 1.3; letter-spacing: 0.025em; }
            `}</style>
        </div>
    );
};
